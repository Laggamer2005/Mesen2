...
public:
    ...
    static void LuaPushIntValue(lua_State* lua, const std::string& name, int value);
...
private:
    ...
    static std::pair<std::unique_ptr<BaseVideoFilter>, FrameInfo> GetRenderedFrame();
    template<typename T>
    static void GenerateEnumDefinition(lua_State* lua, const std::string& enumName, std::unordered_set<T> excludedValues = {});
};
